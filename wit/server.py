...
def sendRequest(protocol,number, service, subservice, location, time):
  r = requests.post("http://139.59.210.181:8888/api/ask/", headers={'content-type': 'application/json'}, data=json.dumps({"time": str(time), "number": str(number), "locations": [str(location)], "protocol": str(protocol), "subservices": str(subservice), "service": str(service)}))
